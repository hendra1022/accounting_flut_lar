...
class SalesLineTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<SalesCreateController>(
      builder: (controller) => controller.isLoading
          ? const VLoadingPage()
          : controller.dataSource.salesLineList.isEmpty
  ...
  DataColumn tableColumn<T>(
    SalesCreateController controller,
    String title,
    Comparable<T> Function(SalesLineRequest user)? sortBy, {
...
class SalesLineDataTableSource extends DataTableSource {
  ...
  void sort<T>(Comparable<T> Function(SalesLineRequest d) getField, bool ascending) {
    salesLineList.sort((SalesLineRequest a, SalesLineRequest b) {
      if (!ascending) {
        final SalesLineRequest c = a;
        a = b;
        b = c;
      }
  ...
  @override
  int get rowCount => salesLineList.length;
  @override
  ...
  DataRow getRow(int index) {
    return DataRow(
      ...
      cells: [
        ...
        dataCell(
          salesLineList[index].itemName,
          Get.width * (24 / 100),
        ),
        dataCell(
          salesLineList[index].unitPrice!.thousandSeparator,
          Get.width * (8 / 100),
        ),
        dataCell(
          salesLineList[index].netAmount,
          Get.width * (8 / 100),
          isMoney: true,
        ),
        dataCell(
          (salesLineList[index].qty ?? "0").thousandSeparator,
          Get.width * (8 / 100),
        ),
      ],
    );
